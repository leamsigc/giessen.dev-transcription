...
const { transcriptions, isLoading, error, addTranscription, updateTranscription, deleteTranscription, getTranscription } = useTauriStoreLoad()
  ...
  <UContainer class="py-8">
    <div class="max-w-6xl mx-auto">
      <div class="flex justify-between items-center mb-8">
        <h1 class="text-3xl font-bold">Transcriptions</h1>
      ...
      <div v-else-if="transcriptions.length === 0" class="text-center py-12">
        ...
        <p class="text-gray-600 mb-4">Start by recording some audio to create your first transcription.</p>
        <NuxtLink to="/" class="inline-block">
          <UButton color="primary">Go to Recorder</UButton>
        </NuxtLink>
      </div>
        ...
        <UCard v-for="transcription in transcriptions" :key="transcription.id" class="p-6">
          <div class="flex justify-between items-start mb-4">
            <div class="flex-1">
              ...
              <div v-if="editingTranscription === transcription.id" class="space-y-3">
                <div class="border rounded-lg p-4 bg-gray-50">
                  <div class="flex items-center justify-between mb-3">
                    <h4 class="font-medium">Visual Editor</h4>
                    <div class="flex gap-2">
                      <UButton @click="formatText('bold')" size="xs" variant="outline">
                        <UIcon name="i-heroicons-bold" class="w-4 h-4" />
                      </UButton>
                      <UButton @click="formatText('italic')" size="xs" variant="outline">
                        <UIcon name="i-heroicons-italic" class="w-4 h-4" />
                      </UButton>
                      <UButton @click="insertTimestamp" size="xs" variant="outline">
                ...
                <div class="flex gap-2">
                  <UButton @click="saveEdit" size="sm" color="primary">
                    <UIcon name="i-heroicons-check" class="w-4 h-4 mr-1" />
                    Save
                  </UButton>
                  <UButton @click="cancelEdit" size="sm" variant="outline">
              ...
              <!-- View Mode -->
              <div v-else>
                <p class="text-gray-700 whitespace-pre-wrap mb-4">{{ transcription.text }}</p>
            ...
            <div v-if="editingTranscription !== transcription.id" class="flex gap-2 ml-4">
              <UDropdown :items="getExportMenuItems(transcription.id)">
                <UButton size="sm" variant="outline" color="green">
                  <UIcon name="i-heroicons-arrow-down-tray" class="w-4 h-4 mr-1" />
                  Export
                </UButton>
              </UDropdown>
              <UButton @click="startEditing(transcription.id)" size="sm" variant="outline">
                <UIcon name="i-heroicons-pencil" class="w-4 h-4" />
              </UButton>
              <UButton @click="confirmDelete(transcription.id)" size="sm" variant="outline" color="red">
      ...
      <UModal v-model="showCreateModal">
        <UCard class="p-6">
          <template #header>
            <h3 class="text-lg font-semibold">Create New Transcription</h3>
          </template>

          <div class="space-y-4">
            <UTextarea
              v-model="newTranscriptionText"
              :rows="6"
          ...
          <template #footer>
            <div class="flex justify-end gap-2">
              ...
              <UButton @click="createTranscription" color="primary">Create</UButton>
            </div>
          </template>
        </UCard>
      </UModal>
