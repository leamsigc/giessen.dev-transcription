...
import { useTauriStoreLoad } from '~/composables/useTauriStoreLoad'

const { app } = useAppConfig()
const { run, isRunning, status, progress, error, result, modelsOptions, selectedModel, getModels } = useTranscriber()
const { selectedMicrophone, getMicrophoneStream, loadMicrophoneSelection } = useMicrophone()
const { addTranscription } = useTauriStoreLoad()
			...
			<div class="flex flex-col items-center gap-y-3">
				...
				<div class="flex flex-col items-center gap-4">
					<div class="flex gap-2">
						<UButton @click="startRecording" :disabled="isRecording" color="primary" size="lg">
							<UIcon name="i-heroicons-microphone" class="w-5 h-5 mr-2" />
							Start Recording
						</UButton>
						<UButton @click="stopRecording" :disabled="!isRecording" color="error" size="lg">
					...
					<div v-if="isRecording" class="recording-view p-6 bg-red-50 rounded-lg border-2 border-red-200">
						<p class="text-red-600 font-semibold mb-4">🔴 Recording...</p>
						<canvas ref="waveCanvas" width="400" height="100" class="border rounded"></canvas>
					</div>

					<!-- Transcription Result -->
					<div v-if="transcriptionResult"
						class="transcription-result p-4 bg-green-50 rounded-lg border border-green-200 max-w-md">
						<p class="font-semibold text-green-800 mb-2">Transcription:</p>
						<p class="text-green-700">{{ transcriptionResult }}</p>
					</div>
					...
					<div class="model-selection">
						...
						<USelect v-model="selectedModel" value-key="value" :items="items" placeholder="Choose a model"
							class="w-64" />
					</div>
				</div>
			</div>
